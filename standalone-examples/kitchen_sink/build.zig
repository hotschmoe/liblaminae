...
pub fn build(b: *std.Build) void {
    ...
    const exe = b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            ...
            .target = target,
            .optimize = optimize,
            .strip = true, // Strip debug info — critical for size
        }),
    });

    // Import liblaminae module
    exe.root_module.addImport("liblaminae", liblaminae.module("liblaminae"));
    ...
    const bin_install = b.addInstallBinFile(objcopy.getOutput(), "kitchen_sink.bin");
    b.getInstallStep().dependOn(&bin_install.step);

    // Create a step to output the binary location
    const install_step = b.getInstallStep();
    const print_step = b.step("build", "Build the kitchen sink demo for Laminae OS");
